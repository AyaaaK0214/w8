...
function deformedSquare(x,y){
    ...
    //a
    cp[0] = rotateAround(v[0],-xSgn*theta,midPoint(v[0],v[1]));
    cp[1] = rotateAround(v[1],-xSgn*theta,midPoint(v[0],v[1]));
    //b
    cp[2] = rotateAround(v[1],-ySgn*theta,midPoint(v[1],v[2]));
    cp[3] = rotateAround(v[2],-ySgn*theta,midPoint(v[1],v[2]));
    //c
    cp[4] = rotateAround(v[2],-xSgn*theta,midPoint(v[2],v[3]));
    cp[5] = rotateAround(v[3],-xSgn*theta,midPoint(v[2],v[3]));
    //d
    cp[6] = rotateAround(v[3],-ySgn*theta,midPoint(v[3],v[0]));
    cp[7] = rotateAround(v[0],-ySgn*theta,midPoint(v[3],v[0]));
